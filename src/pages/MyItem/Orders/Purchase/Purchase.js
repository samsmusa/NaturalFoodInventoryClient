...
import PurchaseCart from '../../../../component/PurchaseCart/PurchaseCart';
...
const Purchase = () => {
  ...
  useEffect(() => {
    fetch(
      "https://raw.githubusercontent.com/samsmusa/natural--inventory/master/public/fakedbPurchase.json"
    )
      .then((res) => res.json())
      .then((res) => {
        setAllData(res);
            ...
            <ul className="list-group scrollspy-example">
              {alldata.length === 0 ? (
                ...
                alldata.map((item, index) => (
                  ...
                  >
                    <PurchaseCart key={index} data={item} />
                  </motion.li>
                ))
              )}
            </ul>
