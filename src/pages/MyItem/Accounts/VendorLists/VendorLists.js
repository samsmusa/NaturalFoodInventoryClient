...
import MyVendors from '../../../../component/MyVendors/MyVendors';
...
const VendorLists = () => {
  ...
  useEffect(() => {
    fetch(
      "https://raw.githubusercontent.com/samsmusa/natural--inventory/master/public/fakedbvendor.json"
    )
      .then((res) => res.json())
      .then((res) => {
        setAllData(res);
            ...
            <ul className="list-group scrollspy-example">
              {alldata.length === 0 ? (
                ...
                alldata.map((item, index) => (
                  ...
                  >
                    <MyVendors key={index} vendor={item} />
                  </motion.li>
                ))
              )}
            </ul>
